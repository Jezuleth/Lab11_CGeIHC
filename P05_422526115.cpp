...
int main()
{
	...
	//Asignamos la carga y ruta de modelos
	...
	while (!mainWindow.getShouldClose())
	{
		...
		//Cuerpo
		color = glm::vec3(0.86f, 0.08f, 0.24f); // Color carro base
		model = glm::mat4(1.0);
		model = glm::translate(model, glm::vec3(0.0f, -2.0f, 0.0f)); // Bajamos el carro
		...
		//Capo
		color = glm::vec3(1.0f, 0.41f, 0.71f); //color capo 
		model = modelCuerpo;
		model = glm::translate(model, glm::vec3(1.58f, 1.0f, 0.0f));
		model = glm::rotate(model, glm::radians(mainWindow.getarticulacion2()), glm::vec3(0.0f, 0.0f, 1.0f));
		glUniform3fv(uniformColor, 1, glm::value_ptr(color));
		glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));
		capo_carro.RenderModel();

		//Rueda izquierda frontal
		color = glm::vec3(1.0f, 0.8f, 0.86f);
		model = modelCuerpo;
