...
void Window::ManejaTeclado(GLFWwindow* window, int key, int code, int action, int mode)
{
	...
	if (key == GLFW_KEY_G)
	{
		static bool incrementando2 = true; // Controla si aumenta o disminuye

		if (incrementando2)
		{
			if (theWindow->articulacion2 < 90) // Límite superior a 90°
			{
				theWindow->articulacion2 += 5.0;
			}
			else
			{
				incrementando2 = false; // Cambia dirección cuando llega a 90°
			}
		}

		if (!incrementando2)
		{
			if (theWindow->articulacion2 > 0) // Vuelve a 0°
			{
				theWindow->articulacion2 -= 5.0;
			}
			else
			{
				incrementando2 = true; // Cambia dirección cuando llega a 0°
			}
		}
	}
	...
	//Apagar y encender luz
	if (key == GLFW_KEY_Z && action == GLFW_PRESS)
	{
		static int estado = 1;

		// Cambia entre 0 y 1 cada vez que se presiona la tecla
		estado = 1 - estado;

		// Actualiza el valor de articulacion13
		theWindow->articulacion13 = estado;
	}
	...
	if (key == GLFW_KEY_X && action == GLFW_PRESS)
	{
		...
		// Actualiza el valor de articulacion11
		theWindow->articulacion11 = estado;
	}
